      ...
      $busqueda = strtolower($_REQUEST['busqueda']);
          ...
          if(empty($busqueda)){
            ...
            ?>  
                 ...
                 <?php   
      $filtro = strtolower($_REQUEST['estado']);
      
          
            ?> 
            
            
          <form class="ml-auto search-form d-none d-md-block" action="buscar_proyectocob.php" method="get">
            <div class="form-group">
              <input name="busqueda" type="search" class="form-control" placeholder="Search Here" value="<?php echo $busqueda; ?>">
            </div>
          </form>
              ...
              <div class="collapse" id="tables">
                <ul class="nav flex-column sub-menu">
                  <li class="nav-item">
                    ...
                    <a class="nav-link" href="listadoproyectoscobranza.php">Servicios Fijos</a>
                  </li>
                  <li class="nav-item">
                ...
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Proyectos</h4>
                      ...
                      <a href="generar_excel.php"><h4 class="card-title">generar excel</h4></a>
                      
                         <form class="ml-auto search-form d-none d-md-block" action="filtrar.php" method="get">

                              <select name="estado" class="mdb-select md-form colorful-select dropdown-primary" id="estado">
                                <option>activo</option>
                                <option>finalizado</option>
                               <option value="<?php echo $filtro; ?>"> </option>
                              </select>
...
<button type="submit" class="btn btn-success mr-2">filtrar</button>
        ...
        $query6="SELECT SUM(VALOR_FACTURADO) AS CN FROM  CONCEPTO, INFORME_DE_PAGO where INFORME_DE_PAGO.CP = CONCEPTO.CP AND CONCEPTO.CP = ".$fila['CP'];
                       $resultado6 = mysqli_query($conexion, $query6);
                          ...
                          $query7="SELECT * FROM estado_cobranza, concepto WHERE estado_cobranza.ID_EO_COB = concepto.ID_EO_COB AND concepto.CP = ".$fila['CP'];
                       $resultado7 = mysqli_query($conexion, $query7);
    
                
                            while($row7 = mysqli_fetch_array($resultado7)){
                            echo $row7['NOM_EO_COB']."</td>";
                            
                            }
    
                        echo  "<td><a href='formeditproyectoscobranza.php?cp=".$fila['CP']."'><button type='button' class='btn btn-warning'> Editar </button></a> </ln></ln>";
    
                         echo "<a href= 'emitirip.php?cp=".$fila['CP']."'><button type='button' class='btn btn-success mr-2'> Emitir IP </button></a> </td>";
                        echo "</tr>";    
